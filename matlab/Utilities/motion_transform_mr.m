...
function [ motion_corrupted_slices, ...
          translation_displacement, ...
                    rotation_angle, ...
                     rotation_axis] = motion_transform_mr(Motion,NbSlices,varargin)
...
%  4 -H yper motion:    translation will vary between [-10,+10]mm and
%                       rotation between [-45,+45]°
% 5 - Custom motion:    custom motion


% List of slices corrupted by motion
corrupted_slice_nb = ceil(NbSlices*Motion.CorruptedSlicesRatio);
...
for iSlice=1:length(motion_corrupted_slices)
    ...
    translation_displacement(iSlice,:) = [unifrnd(-Motion.TranslationAmplitudeX,Motion.TranslationAmplitudeX) ...
                                          unifrnd(-Motion.TranslationAmplitudeY,Motion.TranslationAmplitudeY) ...
                                          unifrnd(-Motion.TranslationAmplitudeZ,Motion.TranslationAmplitudeZ)];    %in mm

    % Rotation
    rotation_angle(iSlice) = unifrnd(-Motion.RotationAmplitude,Motion.RotationAmplitude);   %in degrees
