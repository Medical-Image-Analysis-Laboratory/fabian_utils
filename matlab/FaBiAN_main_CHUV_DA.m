...
% DerivativesRefT2mapPathReo = strcat(DerivativesPathReo, 't2map/', num2str(SubID), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
% 
...
% end
% if not(isfolder(DerivativesRefT2mapPathReo))
%     mkdir(DerivativesRefT2mapPathReo)
% end
% 
% % output_im = strcat(output_path, char(sub_id), '_ses-', sprintf('%02s', num2str(ses_id)), '_run-', num2str(run_id), '_T2w.nii');
% OutputIm = strcat(OutputPath, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_T2w_TE_', num2str(TEeff), '.nii');
% OutputImReo = strcat(OutputPathReo, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_T2w_reo_TE_', num2str(TEeff), '.nii');
...
% OutputImCrop = strcat(OutputPathReo, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_T2w_crop_TE_', num2str(TEeff), '.nii');
% % Derivatives
% OutputLabels = strcat(DerivativesLabelsPath, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_labels_TE_', num2str(TEeff), '.nii');
% OutputLabelsReo = strcat(DerivativesLabelsPathReo, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_labels_reo_TE_', num2str(TEeff), '.nii');
...
% OutputLabelsCrop = strcat(DerivativesLabelsPathReo, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_labels_crop_TE_', num2str(TEeff), '.nii');
% OutputMask = strcat(DerivativesMasksPath, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_mask_TE_', num2str(TEeff), '.nii');
% OutputMaskReo = strcat(DerivativesMasksPathReo, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_mask_reo_TE_', num2str(TEeff), '.nii');
...
% OutputMaskCrop = strcat(DerivativesMasksPathReo, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_mask_crop_TE_', num2str(TEeff), '.nii');
% 
% OutputRefT2map = strcat(DerivativesRefT2mapPathReo, num2str(SubID), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_reft2map.nii');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Create output folders / filenames     MARGAUX                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Create output folders to organize the data into BIDS
% Images folder
OutputPath = strcat(OutputFolder, 'data/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
OutputPathCrop = strcat(OutputFolder, 'data_crop/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
% OutputPathSimParams = strcat(OutputFolder, 'data_crop/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/sim_params/');
OutputPathReo = strcat(OutputFolder, 'data_reo/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');

if not(isfolder(OutputPath))
    mkdir(OutputPath)
end
if not(isfolder(OutputPathCrop))
    mkdir(OutputPathCrop)
end
if not(isfolder(OutputPathReo))
    mkdir(OutputPathReo)
end
% if not(isfolder(OutputPathSimParams))
%     mkdir(OutputPathSimParams)
% end

% Derivatives folders
DerivativesPath = strcat(OutputFolder, 'data/derivatives/');
DerivativesPathCrop = strcat(OutputFolder, 'data_crop/derivatives/');
DerivativesPathReo = strcat(OutputFolder, 'data_reo/derivatives/');

if not(isfolder(DerivativesPath))
    mkdir(DerivativesPath)
end
if not(isfolder(DerivativesPathCrop))
    mkdir(DerivativesPathCrop)
end
if not(isfolder(DerivativesPathReo))
    mkdir(DerivativesPathReo)
end
...
% Labels
DerivativesLabelsPath = strcat(DerivativesPath, 'labels/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
DerivativesLabelsPathReo = strcat(DerivativesPathReo, 'labels/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
DerivativesLabelsPathCrop = strcat(DerivativesPathCrop, 'labels/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
% Masks
DerivativesMasksPath = strcat(DerivativesPath, 'masks/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
DerivativesMasksPathReo = strcat(DerivativesPathReo, 'masks/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
DerivativesMasksPathCrop = strcat(DerivativesPathCrop, 'masks/sub-', sprintf('%03s',num2str(SubID)), '/ses-', sprintf('%02s', num2str(SesID)), '/anat/');
% T2map ref (only in data)
DerivativesRefT2mapPath = strcat(DerivativesPathCrop, 't2map/sub-', sprintf('%03s',num2str(SubID)), '/');
...
if not(isfolder(DerivativesLabelsPathCrop))
    mkdir(DerivativesLabelsPathCrop)
end
if not(isfolder(DerivativesMasksPath))
    mkdir(DerivativesMasksPath)
end
...
if not(isfolder(DerivativesMasksPathCrop))
    mkdir(DerivativesMasksPathCrop)
end
if not(isfolder(DerivativesRefT2mapPath))
    mkdir(DerivativesRefT2mapPath)
end
...
if NbSlices > 200
    error('Volume is too big to run FaBIAN');
end
...
[  MotionCorruptedSlices, ...
 TranslationDisplacement, ...
           RotationAngle, ...
            RotationAxis] = motion_transform_mr(Motion, ...
                                                NbSlices);
...
% save([OutputPathSimParams,'sim_parameters.mat'],                'SubID', ...
%                                           'SesID', ...
%                                           'RunID', ...
%                                      'FetalModel', ...
%                                     'Orientation', ...
...
% % writecell(horzcat(param, struct2cell(data)), strcat(OutputPath, 'sub-', sprintf('%03s', num2str(SubID)), '_ses-', sprintf('%02s', num2str(SesID)), '_run-', num2str(RunID), '_sim_parameters.csv'))
